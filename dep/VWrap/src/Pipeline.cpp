...
namespace VWrap {
    ...
    std::shared_ptr<Pipeline> Pipeline::CreateRasterizer(std::shared_ptr<Device> device, std::shared_ptr<RenderPass> render_pass, std::shared_ptr<DescriptorSetLayout> descriptor_set_layout, VkExtent2D extent){

        auto ret = std::make_shared<Pipeline>();
        ret->m_device = device;

        auto vertShaderCode = readFile("../shaders/vert_rast.spv");
        auto fragShaderCode = readFile("../shaders/frag_rast.spv");
    ...
    std::shared_ptr<Pipeline> Pipeline::CreateTracer(std::shared_ptr<Device> device, std::shared_ptr<RenderPass> render_pass, std::shared_ptr<DescriptorSetLayout> descriptor_set_layout, VkExtent2D extent)
    {
        auto ret = std::make_shared<Pipeline>();
        ret->m_device = device;

        auto vertShaderCode = readFile("../shaders/vert_tracer.spv");
        auto fragShaderCode = readFile("../shaders/frag_tracer.spv");
        ...
        pushConstantRange.size = sizeof(PushConstantBlock); // Size of the push constant block
